...
// scenario would happen if the podman-machine was created with the host on one network, and then the host gets
...
func mockCalls(t *testing.T, driver *Driver, expectedCalls []Call) {
	...
	driver.Boot2PodmanURL = "http://b2p.org"
	driver.VBoxManager = mockOperations
	driver.b2pUpdater = mockOperations
...
func TestCreateVM(t *testing.T) {
	...
	mockCalls(t, driver, []Call{
		{"CopyIsoToMachineDir path default http://b2p.org", "", nil},
		...
		{"vbm storageattach default --storagectl SATA --port 0 --device 0 --type dvddrive --medium path/machines/default/boot2podman.iso", "", nil},
