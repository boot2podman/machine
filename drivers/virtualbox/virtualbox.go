...
// GetCreateFlags registers the flags this driver adds
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	...
	d.Boot2PodmanURL = flags.String("virtualbox-boot2podman-url")
	d.SSHUser = "tc"
...
func (d *Driver) PreCreateCheck() error {
	...
	// Downloading boot2podman to cache should be done here to make sure
	// that a download failure will not leave a machine half created.
	if err := d.b2pUpdater.UpdateISOCache(d.StorePath, d.Boot2PodmanURL); err != nil {
		return err
	}
...
func (d *Driver) CreateVM() error {
	if err := d.b2pUpdater.CopyIsoToMachineDir(d.StorePath, d.MachineName, d.Boot2PodmanURL); err != nil {
		return err
	}
	...
	if err := d.vbm("storageattach", d.MachineName,
		...
		"--device", "0",
		"--type", "dvddrive",
		"--medium", d.ResolveStorePath("boot2podman.iso")); err != nil {
		return err
	}
...
func (d *Driver) parseIPForMACFromIPAddr(ipAddrOutput string, macAddress string) (string, error) {
	// Given the output of "ifconfig" on the VM, return the IPv4 address
	...
	for _, line := range lines {
		...
		if strings.Contains(line, "Link") { // line contains MAC address
			vals := strings.Split(line, "HWaddr ")
			if len(vals) >= 2 {
...
func (d *Driver) GetIP() (string, error) {
	...
	output, err := drivers.RunSSHCommandFromDriver(d, "ifconfig")
	if err != nil {
		return "", err
	}
