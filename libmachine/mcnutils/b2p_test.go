...
func TestGetReleaseURL(t *testing.T) {
	testCases := []struct {
		...
		{"/repos/org/repo/releases/latest", "/org/repo/releases/download/v0.1/boot2podman.iso", "v0.7.0", `{"tag_name": "v0.1"}`},

		// Note the difference in this one: It's an RC version.
		{"/repos/org/repo/releases", "/org/repo/releases/download/v0.2-rc1/boot2podman.iso", "v0.7.0-rc2", `[{"tag_name": "v0.2-rc1"}, {"tag_name": "v0.1"}]`},

		{"http://dummy.com/boot2podman.iso", "http://dummy.com/boot2podman.iso", "v0.7.0", `{"tag_name": "v0.1"}`},
	}
...
func TestCopyDefaultISOToMachine(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		// isopath: "$TMPDIR/machine-test-xxxxxx/cache/boot2podman.iso"
...
func newDummyISO(dir, name, version string) (string, int64, error) {
	...
	// dummy ISO data mimicking the real byte data of a Boot2Podman ISO image
	padding := "     "
...
func dummyISOData(padding, version string) []byte {
	return []byte(fmt.Sprintf("%sBoot2Podman-%s                    ", padding, version))
}
